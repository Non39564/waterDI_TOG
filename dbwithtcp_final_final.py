...
def water_check(Site,min,max,water,date,time,connection_check):
    if connection_check is True: #min = 10 , max = 20
...
def append_data_before(OP,Phase,Site,Temp,Water,offset,connection):
    if Water < 12:
        status = 'L'
    elif 12 <= Water <= 13:
        status = 'M'
    ...
    if len(datalist) == 0 :
        datalist.append({"OP":OP,"Phase":Phase,"Site":Site,"Temp":Temp,"Water":Water,"Status":status,"Alarm":"OFF","Count": 0})
    else:
        count = 0
        for dl in range(len(datalist)):
            if datalist[dl]["OP"] == OP and datalist[dl]["Phase"] == Phase and datalist[dl]["Site"] == Site:
                if datalist[dl]["Status"] == status and  status != 'N':
                    if datalist[dl]["Alarm"] == "OFF":
                        if datalist[dl]["Count"] >= 10:
                            datalist[dl]["Alarm"] = "ON"
                            Alarm(Site,offset,Water,Temp,connection)
                        else:
                            datalist[dl]["Count"] = datalist[dl]["Count"] +1
                    else:
                        pass
                else:
                    datalist[dl]["Count"] = 0
                    datalist[dl]["Alarm"] = "OFF"
                
                datalist[dl]["Status"] = status
                datalist[dl]["Water"] = Water
                count =+1     
        if count == 0:
            datalist.append({"OP":OP,"Phase":Phase,"Site":Site,"Temp":Temp,"Water":Water,"Status":status,"Alarm":"OFF","Count": 0})
                ...
                for k in range(len(ip_port)):
                    if m['Ip']+":"+str(m['Port']) == ip_port[k]:
                        ...
                        if water_data == -1 and temp_data == -1:
                            ...
                            append_data_before(m['OP'],m['phase'],m['Site'],temp_data,water_data,offset,connection_check[k])
